...
import { defaultData,addUpdateStudent,getStudentDetails } from '@/api/student'
...
export default class extends Vue {
    ...
    try {
      const { data } = await getStudentDetails(id)
        ...
        if (userObj){
           ...
           this.postForm.StudentId=data.studentId
  ...
  private submitForm() {
    (this.$refs.studentPostForm as Form).validate( async (valid) => {
      ...
      if (valid) {
        ...
        if (this.isEdit) {
            formdata.append("StudentId",this.postForm.StudentId.toString())
        }
        ...
        if (this.isEdit) {
          const {data} = await addUpdateStudent(formdata)
          this.$notify({
          title: 'Success',
